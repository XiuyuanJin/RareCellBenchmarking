...
## -----------------------------
start_time   <- Sys.time()
data_seurat  <- cluster_analysis_integrate_rare(read.counts, experimentID, 0.1, 5, 30)
...
data_ciara  <- cluster_analysis_integrate_rare(read.counts, experimentID, 0.01, 5, 30, feature_genes = ciara_genes)
end_time      <- Sys.time()
...
## Save result
## -----------------------------
write.csv(result,file = paste0("/results/CIARA_", Celltype, ".csv"),row.names = TRUE)
